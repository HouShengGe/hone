...
/**
 * Created by hexio on 2017/7/20.
 */

public class MyTimeSelect {
    ...
    public MyTimeSelect(Context context, ResultHandler resultHandler, String startDate, String endDate) {
    ...
    public MyTimeSelect(Context context, ResultHandler resultHandler, String startDate, String endDate, String workStartTime, String workEndTime) {
    ...
    private void initTimer() {
        ...
        if (spanYear) {
            ...
            for (int i = 1; i <= MAXMONTH; i++) {
                ...
                if (moth<=MAXMONTH) {
                    month.add(fomatTimeUnit(moth));
                }else{
                    month.add(fomatTimeUnit(moth-MAXMONTH));
                }
                moth++;
            }
            int days = startDay;
            for (int i = 1; i <= selectedCalender.getActualMaximum(Calendar.DAY_OF_MONTH); i++) {
                if (days<=selectedCalender.getActualMaximum(Calendar.DAY_OF_MONTH)) {
                    day.add(fomatTimeUnit(days));
                }else{
                    day.add(fomatTimeUnit(days-selectedCalender.getActualMaximum(Calendar.DAY_OF_MONTH)));
                }
                days++;
            }
...
//                }
    ...
    private boolean excuteWorkTime() {
        boolean res = true;
        if (!StringUtil.isBlank(workStart_str) && !StringUtil.isBlank(workEnd_str)) {
